...
import { ArrowRight, BarChart3, Bot, Calculator, CheckCircle, Cpu, Crown, Gift, Instagram, Lock, Menu,
  MessageCircleCode,
  Palette, Play, Rocket, Settings, Share2, Shield, Star, Users, Wand2, X, Zap } from "lucide-react";
    ...
    <div className="relative bg-white rounded-3xl p-6 border border-gray-100 shadow-sm hover:border-indigo-200 transition-colors duration-300 h-full overflow-hidden group">
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-4">
          ...
          <span className={`text-[10px] font-bold px-3 py-1.5 rounded-full ${feature.tag === "GRÁTIS" ? "bg-green-100 text-green-700" : feature.tag === "PRO" ? "bg-blue-100 text-blue-700" : "bg-purple-100 text-purple-700"}`}>
            {feature.tag}
          </span>
        </div>
...
const HowItWorksStep = ({ step, index, total }: { step: { icon: React.ReactNode; title: string; desc: string }; index: number; total: number }) => {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 50 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: index * 0.2, duration: 0.6 }} className="relative">
      {index < total - 1 && (
        <motion.div className="hidden lg:block absolute top-14 left-1/2 w-full h-1 overflow-hidden" initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}}>
          <motion.div className="h-full bg-gradient-to-r from-indigo-400 to-purple-400" initial={{ width: 0 }} animate={isInView ? { width: "100%" } : {}} transition={{ delay: index * 0.2 + 0.5, duration: 0.8 }} />
        </motion.div>
      )}
      <div className="relative z-10 flex flex-col items-center text-center">
        <MagneticWrapper>
          <motion.div whileHover={{ scale: 1.1, rotate: [0, -5, 5, 0] }} whileTap={{ scale: 0.95 }} className="w-28 h-28 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-[2rem] flex items-center justify-center text-white shadow-xl shadow-indigo-500/30 mb-4 cursor-pointer relative overflow-hidden">
            <motion.div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/20 to-white/0" animate={{ x: ["-100%", "100%"] }} transition={{ repeat: Infinity, duration: 2, ease: "linear" }} />
            <span className="relative z-10">{step.icon}</span>
          </motion.div>
        </MagneticWrapper>
        <motion.div whileHover={{ scale: 1.1 }} className="bg-gradient-to-r from-indigo-100 to-purple-100 text-indigo-700 text-sm font-bold px-4 py-1.5 rounded-full mb-3">
          Passo {index + 1}
        </motion.div>
...
const NichoCard = ({ nicho }: { nicho: typeof nichos[0] }) => (
  <ScrollReveal>
    <div className="bg-white rounded-2xl p-4 border border-gray-100 hover:border-indigo-200 transition-colors duration-300 cursor-pointer text-center group h-full">
      ...
      <p className="text-xs sm:text-sm font-bold text-gray-700 group-hover:text-indigo-700 transition-colors">{nicho.name}</p>
    </div>
  </ScrollReveal>
);

const DifferentialCard = ({ item }: { item: { icon: React.ReactNode; title: string; desc: string } }) => (
  <ScrollReveal>
    <div className="bg-white/5 backdrop-blur-sm rounded-3xl p-6 border border-white/10 hover:bg-white/10 transition-colors duration-300 h-full">
      <div className="w-14 h-14 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-2xl flex items-center justify-center mb-4 shadow-lg">{item.icon}</div>
...
export default function LandingPage() {
  ...
  return (
    <div className="bg-white text-gray-900 font-sans selection:bg-indigo-500 selection:text-white w-full">
      <motion.div className="fixed top-0 left-0 right-0 h-1 bg-gradient-to-r from-indigo-500 to-purple-500 z-[100] origin-left" style={{ scaleX: scrollYProgress }} />
      <WhatsAppFloatingButton />

      <AnimatePresence>
        {showStickyFooter && (
          <motion.div initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }} className="fixed bottom-0 left-0 right-0 z-[90] p-3 bg-white/95 backdrop-blur-md border-t border-gray-200 md:hidden shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
      ...
      <nav className={`fixed top-0 w-full z-[80] transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-sm shadow-sm" : "bg-transparent"}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 md:h-20 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-8">
            {[{ label: "Funcionalidades", href: "#funcionalidades" }, { label: "Como Funciona", href: "#como-funciona" }, { label: "Preços", href: "#precos" }, { label: "Depoimentos", href: "#depoimentos" }].map((item) => (
              <a key={item.label} href={item.href} className="text-sm font-medium text-gray-600 hover:text-indigo-600 transition-colors">{item.label}</a>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3 relative z-50">
            <div className="hidden lg:block">
              <SignInButton mode="modal">
                <button className="text-sm font-bold text-gray-600 hover:text-indigo-600 transition-colors px-4 py-2 cursor-pointer">Entrar</button>
              </SignInButton>
            </div>
          ...
          {mobileMenuOpen && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="lg:hidden bg-white border-b border-gray-100 overflow-hidden">
              <div className="px-4 py-4 space-y-1">
                {[{ label: "Funcionalidades", href: "#funcionalidades" }, { label: "Como Funciona", href: "#como-funciona" }, { label: "Preços", href: "#precos" }, { label: "Depoimentos", href: "#depoimentos" }].map((item) => (
                  <a key={item.label} href={item.href} onClick={() => setMobileMenuOpen(false)} className="block py-3 px-4 text-gray-700 hover:bg-gray-50 rounded-xl font-medium">{item.label}</a>
                ))}
                <div className="pt-2">
                  <SignInButton mode="modal">
                    <button className="block w-full py-3 px-4 text-center bg-gray-50 text-indigo-600 font-bold rounded-xl cursor-pointer">Entrar na Plataforma</button>
                  </SignInButton>
                </div>
              </div>
            </motion.div>
            ...
            <div className="text-center lg:text-left">
              <ScrollReveal>
                <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-50 border border-green-200 rounded-full text-sm font-bold text-green-700 mb-6">
                  <Gift size={16} /> Teste 7 dias grátis — Todas as ferramentas
                </div>
                <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-black tracking-tight leading-[1.1] mb-6">
                  Transforme seu Link na Bio em uma <span className={BRAND.textGradient}>Máquina de Vendas</span>
                </h1>
                <p className="text-lg sm:text-xl text-gray-600 max-w-xl mx-auto lg:mx-0 mb-8 leading-relaxed">
                  Página totalmente customizável, encurtador com analytics e ferramentas de <strong className="text-indigo-600">IA exclusivas</strong>. Pare de perder vendas no Instagram.
                </p>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <ScrollReveal>
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-gray-900 mb-4">Veja o que nossos usuários <span className={BRAND.textGradient}>criaram</span></h2>
              <p className="text-gray-600 text-lg max-w-2xl mx-auto">100% customizável. Você escolhe cores, fontes, layout e muito mais.</p>
            </div>
          </ScrollReveal>
          <RealPagesShowcase />
        ...
        <div className="max-w-7xl mx-auto px-4">
          <ScrollReveal>
            <div className="text-center mb-16">
              ...
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-gray-900 mb-4">Tudo que você precisa para <span className={BRAND.textGradient}>vender mais</span></h2>
              <p className="text-gray-600 text-lg max-w-2xl mx-auto">Não somos só um link na bio. Somos seu sistema de vendas completo.</p>
            </div>
          </ScrollReveal>
      ...
      <section id="como-funciona" className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-4">
            {howItWorks.map((step, i) => <HowItWorksStep key={i} step={step} index={i} total={howItWorks.length} />)}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <ScrollReveal>
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-gray-900 mb-4">Serve pra você? <span className={BRAND.textGradient}>Com certeza.</span></h2>
            </div>
          </ScrollReveal>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            {nichos.map((nicho, i) => <NichoCard key={i} nicho={nicho} />)}
          </div>
        </div>
      </section>
      ...
      <section id="diferenciais" className="py-24 bg-gray-900 text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          <ScrollReveal>
            <div className="text-center mb-16">
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-black mb-4">Por que <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">Freelinnk</span>?</h2>
            </div>
          </ScrollReveal>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {differentials.map((item, i) => <DifferentialCard key={i} item={item} />)}
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-2xl mx-auto px-4 text-center">
          <ScrollReveal>
            ...
            <p className="text-gray-600 text-lg leading-relaxed mb-8">Use tudo. Se em 7 dias você não sentir que o valor entregue é <strong className="text-indigo-600">10x maior</strong> que o preço, devolvemos 100% do seu dinheiro. <strong>Sem perguntas.</strong></p>
          </ScrollReveal>
        </div>
      </section>
        ...
        <div className="max-w-3xl mx-auto px-4">
          ...
          <div className="space-y-4">
            {[
              ...
              { q: "Como funciona o teste grátis de 7 dias?", a: "Você tem acesso completo a todas as funcionalidades do plano escolhido por 7 dias. Se não gostar, cancele antes do período acabar." }
            ].map((faq, i) => (
              <ScrollReveal key={i}>
      ...
      <section className="py-28 bg-gradient-to-br from-indigo-600 via-purple-600 to-indigo-700 text-white relative overflow-hidden">
        <div className="max-w-4xl mx-auto px-4 text-center relative z-20">
          <ScrollReveal>
            <h2 className="text-4xl sm:text-5xl md:text-6xl font-black mb-6 leading-tight">Pronto para transformar seu perfil em um <span className="text-yellow-300">negócio?</span></h2>
            <SignInButton mode="modal">
          ...
          <div className="grid md:grid-cols-4 gap-12 mb-12">
            <div className="md:col-span-2">
              <div className="flex items-center gap-2.5 mb-6">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-white font-black text-2xl ${BRAND.gradient} shadow-lg`}>
                  F
                </div>
              ...
              <div className="flex gap-4">
                {[
                  {
                    url: "https://www.instagram.com/lucas.aragaotm",
