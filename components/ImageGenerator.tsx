...
import { useAction, useQuery, useMutation } from "convex/react"; // 1. MUDANÇA AQUI
...
import {
  ...
  Grid3x3, ImageIcon, Camera, Shapes, X, ArrowLeft,
  ...
  Palette, Trophy, Target, Lightbulb, MessageCircle, MoreVertical,
  Trash2 // 2. MUDANÇA AQUI
} from "lucide-react";
...
export function ImageGenerator() {
  ...
  const [deletingImageId, setDeletingImageId] = useState<string | null>(null); // 3. MUDANÇA AQUI
  ...
  const deleteImageMutation = useMutation(api.imageGenerator.deleteImage); // 4. MUDANÇA AQUI
  ...
  // 5. NOVA FUNÇÃO DE DELETAR
  const handleDeleteImage = async (imageId: Id<"generatedImages">, storageId: Id<"_storage">) => {
    if (deletingImageId === imageId) return; // Já está deletando
    ...
    const toastId = toast.loading("Deletando imagem...");

    try {
      await deleteImageMutation({ imageId, storageId });
      toast.dismiss(toastId);
      toast.success("Imagem deletada com sucesso! 🗑️");
      // O 'imageHistory' (useQuery) será atualizado automaticamente pelo Convex
    } catch (error) {
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm text-purple-700 mt-1">
                  Digite sua ideia, escolha um estilo e veja a mágica acontecer! Nossa IA avançada cria imagens profissionais em segundos.
                </p>
              </div>
            </motion.div>
            <motion.div
...
ROTEIRO COMPLETO:
${videoScript.scenes.map(s => `Cena ${s.number} (${s.duration}): \nTexto: ${s.text} \nVisual: ${s.visual} \nCâmera: ${s.camera} \nTransição: ${s.transition}`).join('\n\n')}
...
`;
                            ...
                            const a = document.createElement('a');
                            a.href = url; a.download = `roteiro-viral-${Date.now()}.txt`;
                            a.click();
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => setShowProTips(!showProTips)}>
                              <Lightbulb className="w-4 h-4 mr-2" />
                              {showProTips ? 'Ocultar' : 'Ver'} Dicas Pro
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => setVideoScript(null)}>
                          ...
                          <div className={`absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent ${isMobile ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-all duration-300 flex flex-col justify-end p-3`}>
                            ...
                            <div className="flex justify-between items-center">
                              <div className="flex gap-1">
                                ...
                                {/* 6. BOTÃO DE DELETAR ADICIONADO AQUI */}
                                <motion.button
                                  whileTap={{ scale: 0.9 }}
                                  onClick={() => handleDeleteImage(image._id, image.storageId)}
                                  disabled={deletingImageId === image._id}
                                  className="p-2 bg-red-500/20 backdrop-blur rounded-lg hover:bg-red-500/40 transition-colors"
                                  aria-label="Deletar"
                                >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-white p-4 rounded-xl text-center">
                      <p className="text-3xl font-bold text-orange-600">
                        {imageHistory.length > 0 ? Math.round((likedImages.size / imageHistory.length) * 100) : 0}%
                      </p>
