// Substituir o componente HourlyChart por completo:
...
export function HourlyChart({ data: hourlyData }: HourlyChartProps) {
  const [hoveredHour, setHoveredHour] = useState<number | null>(null);

  const maxClicks = Math.max(...hourlyData.map(h => h.total_clicks), 1);
    ...
    <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-8 shadow-2xl border border-gray-100">
      ...
      <div className="relative aspect-square max-w-sm mx-auto">
        <svg viewBox="0 0 200 200" className="w-full h-full -rotate-90">
          {hourlyData.map((item) => {
            const angle = (item.hour_of_day / 24) * 360;
            const innerRadius = 50;
            const outerRadius = innerRadius + (item.total_clicks / maxClicks) * 30;
            ...
            return (
              <g key={item.hour_of_day}>
                <line
                  x1={x1}
                  y1={y1}
                  x2={x2}
                  y2={y2}
                  stroke={hoveredHour === item.hour_of_day ? "#8B5CF6" : item.total_clicks > 0 ? "#60A5FA" : "#E5E7EB"}
                  strokeWidth="6"
                  strokeLinecap="round"
                  className="transition-all duration-300 cursor-pointer"
                  onMouseEnter={() => setHoveredHour(item.hour_of_day)}
            ...
            {hoveredHour !== null ? (
              <>
                ...
                <p className="text-sm text-gray-600">
                  {hourlyData.find(h => h.hour_of_day === hoveredHour)?.total_clicks || 0} cliques
                </p>
              </>
            ) : (
              <>
                <p className="text-lg font-bold text-gray-900">24h</p>
                <p className="text-sm text-gray-600">Passe o mouse</p>
              </>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-6 gap-2 mt-6">
        {[0, 4, 8, 12, 16, 20].map(hour => {
          const data = hourlyData.find(h => h.hour_of_day === hour);
          return (
            <div key={hour} className="text-center">
              <p className="text-xs font-semibold text-gray-700">{hour}:00</p>
              <p className="text-xs text-gray-500">{data?.total_clicks || 0} cliques</p>
            </div>
          );
        })}
      </div>
