...
// ... (Mantenha as funções getGreeting e getDailyPerformanceCard IGUAIS ao que você já tem, não precisa alterar essa parte) ...
const getGreeting = () => {
  const date = new Date();
...
const getDailyPerformanceCard = (clicks: number) => {
    // ... (Mantenha seu código atual dessa função aqui) ...
    // Vou omitir para economizar espaço, pois não mudamos a lógica dela
    if (clicks <= 10) {
        return (
...
export default async function DashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto pb-24 pt-4 animate-in fade-in duration-500 space-y-8">
      ...
      {/* 2. ÁREA DE IMPACTO + STATUS DINÂMICO */}
      ...
      {/* 4. DETALHES (Grid Principal) */}
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">

        {/* COLUNA ESQUERDA (Maior) - Agora contém o Checklist também! */}
        <div className="md:col-span-8 space-y-6">
          ...
          {/* NOVO POSICIONAMENTO: Growth Checklist ocupando a largura total da coluna principal */}
          <div>
             <GrowthChecklist plan={userPlan} clicks={totalClicks} username={user.username || "meu-link"} />
          </div>
        </div>

        {/* COLUNA DIREITA (Menor) - Apenas o Trending Link agora */}
        <div className="md:col-span-4 space-y-6">
          <TrendingLinkCard analytics={analytics} plan={userPlan} />
          {/* Se a coluna da direita ficar muito vazia, podemos adicionar um banner de upgrade ou dicas rápidas aqui no futuro */}
        </div>
      </div>
    </div>
  );
