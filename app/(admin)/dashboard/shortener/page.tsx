    ...
    <div className="space-y-8">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-lg border">
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
          <div><Label htmlFor="longUrl" className="font-semibold">URL de Destino</Label><Input id="longUrl" name="originalUrl" type="url" placeholder="https://exemplo-de-link-muito-longo.com/produto" required className="py-6 mt-1" disabled={isLoading} /></div>
          ...
          <Button type="submit" disabled={isLoading} className="w-full sm:w-auto py-3 font-bold">{isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <><LinkIcon className="w-4 h-4 mr-2" /> Encurtar Link</>}</Button>
        </form>
      </div>
      <div>
