    ...
    <DialogContent className="sm:max-w-md p-0 gap-0 overflow-hidden rounded-2xl border-0 shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="p-5 space-y-5">
          <div className="bg-gray-50 rounded-xl p-4">
            ...
            <div className="flex items-center gap-4">
              <div onClick={() => fileInputRef.current?.click()} className={clsx("w-20 h-20 rounded-xl border-2 flex items-center justify-center cursor-pointer transition-all overflow-hidden bg-white", imagePreview ? "border-purple-400 shadow-lg shadow-purple-100" : "border-dashed border-gray-300 hover:border-purple-400")}>
                {imagePreview ? <img src={imagePreview} alt="Preview" className="w-full h-full object-cover" /> : <div className="text-center"><ImageIcon className="w-6 h-6 text-gray-300 mx-auto" /><span className="text-[10px] text-gray-400 mt-1 block">Adicionar</span></div>}
              </div>

              <div className="flex flex-col gap-2">
                <button type="button" onClick={() => fileInputRef.current?.click()} className="px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all">
                  {imagePreview ? 'Trocar imagem' : 'Escolher imagem'}
                </button>
                {imagePreview && (
                  <button onClick={handleRemoveImage} className="px-4 py-2 text-sm font-medium text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-all">
                    Remover
                  </button>
                )}
              </div>
          ...
          <div className="bg-gray-50 rounded-xl p-4">
            ...
            <Input value={title} onChange={(e) => setTitle(e.target.value)} className="h-12 rounded-xl border-gray-200 bg-white text-base font-medium focus:border-purple-400 focus:ring-purple-400" placeholder="Ex: Meu Instagram" />
          </div>
          ...
          <div className="bg-gray-50 rounded-xl p-4">
            ...
            <Input value={url} onChange={(e) => setUrl(e.target.value)} className="h-12 rounded-xl border-gray-200 bg-white text-base focus:border-purple-400 focus:ring-purple-400" placeholder="https://..." />
          </div>

          {showMessageField && (
            <div className={clsx("rounded-xl p-4", isWhatsApp ? "bg-green-50" : "bg-blue-50")}>
              <div className="flex items-center gap-2 mb-3">
                {isWhatsApp ? <MessageCircle className="w-4 h-4 text-green-600" /> : <Mail className="w-4 h-4 text-blue-600" />}
                <span className={clsx("text-sm font-semibold", isWhatsApp ? "text-green-700" : "text-blue-700")}>
                  {isWhatsApp ? "Mensagem do WhatsApp" : "Corpo do E-mail"}
                </span>
              </div>
              <Textarea value={customMessage} onChange={(e) => setCustomMessage(e.target.value)} className={clsx("min-h-[100px] rounded-xl border bg-white text-base resize-none", isWhatsApp ? "border-green-200 focus:border-green-400 focus:ring-green-400" : "border-blue-200 focus:border-blue-400 focus:ring-blue-400")} placeholder="Ex: Olá! Vim pelo seu link..." />
            </div>
          )}
          ...
          <div className="bg-gray-50 rounded-xl p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={clsx("w-10 h-10 rounded-xl flex items-center justify-center", isActive ? "bg-green-100" : "bg-gray-200")}>
                  <Power className={clsx("w-5 h-5", isActive ? "text-green-600" : "text-gray-400")} />
                </div>
                <div>
                  <span className="text-sm font-semibold text-gray-700 block">Link Ativo</span>
                  <span className="text-xs text-gray-500">{isActive ? "Visível na sua página" : "Oculto temporariamente"}</span>
                </div>
              </div>
      ...
      <div className="border-t border-gray-100 bg-white p-4 sticky bottom-0">
        <div className="flex gap-3">
          <DialogClose asChild>
            <Button variant="outline" className="flex-1 h-12 rounded-xl font-semibold border-gray-200 hover:bg-gray-50">Cancelar</Button>
          </DialogClose>
          <Button onClick={handleSave} disabled={isSubmitting} className="flex-1 h-12 rounded-xl font-semibold bg-purple-600 hover:bg-purple-700 text-white shadow-lg shadow-purple-200">
...
function SortableLinkItem({ id, link, onEdit, onDelete }: SortableLinkItemProps) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={clsx("group bg-white border rounded-2xl transition-all duration-200 touch-none overflow-hidden", isDragging ? "shadow-2xl border-purple-300 scale-[1.02]" : "border-gray-100 hover:border-gray-200 hover:shadow-lg", link.active === false && "opacity-50 bg-gray-50/50")}>
      <div className="flex items-center gap-3 p-3 sm:p-4">
        <div {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing text-gray-300 hover:text-gray-400 p-1 -ml-1 touch-none">
        ...
        <div className="w-11 h-11 sm:w-12 sm:h-12 flex-shrink-0 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl overflow-hidden flex items-center justify-center border border-gray-200/50">
          {link.thumbnailUrl ? <Image src={link.thumbnailUrl} alt="icon" width={48} height={48} className="w-full h-full object-cover" /> : <Link2 className="w-5 h-5 text-gray-400" />}
        </div>

        <div className="flex-1 min-w-0 py-0.5">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-gray-900 truncate text-sm sm:text-base">{link.title}</h3>
            <div className={clsx("w-1.5 h-1.5 rounded-full flex-shrink-0", link.active !== false ? "bg-green-500" : "bg-gray-300")} />
          </div>
          <a href={link.url} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs text-gray-500 hover:text-purple-600 transition-colors mt-0.5 group/link">
      ...
      <div className="sm:hidden border-t border-gray-100 bg-gray-50/50">
        <div className="flex items-center divide-x divide-gray-200">
          <Link href={`/dashboard/link/${link._id}`} className="flex-1 flex items-center justify-center gap-2 py-3 text-purple-600 hover:bg-purple-50 active:bg-purple-100 transition-colors">
            <BarChart2 className="w-4 h-4" /><span className="text-sm font-medium">Ver Análises</span><ChevronRight className="w-4 h-4" />
          </Link>
          <button onClick={() => onEdit(link)} className="flex items-center justify-center gap-1.5 py-3 px-4 text-gray-600 hover:bg-gray-100 active:bg-gray-200 transition-colors"><Edit2 className="w-4 h-4" /><span className="text-sm">Editar</span></button>
          <button onClick={() => onDelete(link._id)} className="flex items-center justify-center py-3 px-4 text-red-500 hover:bg-red-50 active:bg-red-100 transition-colors"><Trash2 className="w-4 h-4" /></button>
        </div>
      </div>
    </div>
  );
...
function LinksSkeleton() {
  return (
    <div className="space-y-3 animate-pulse">
      {[...Array(3)].map((_, i) => (
        <div key={i} className="bg-white border border-gray-100 rounded-2xl p-4"><div className="flex items-center gap-3"><div className="w-5 h-8 bg-gray-100 rounded" /><div className="w-11 h-11 bg-gray-100 rounded-xl" /><div className="flex-1 space-y-2"><div className="h-4 bg-gray-100 rounded-lg w-2/3" /><div className="h-3 bg-gray-100 rounded-lg w-1/2" /></div></div></div>
      ))}
    </div>
  );
}
    ...
    <div className="relative overflow-hidden text-center p-6 sm:p-10 border-2 border-dashed border-gray-200 rounded-2xl bg-gradient-to-br from-gray-50/50 to-white">
      ...
      <div className="relative">
        <div className="mx-auto w-fit relative mb-4">
          <div className="relative bg-gradient-to-br from-gray-100 to-gray-50 p-4 rounded-2xl border border-gray-200 shadow-sm"><Link2 className="w-8 h-8 text-gray-400" /></div>
          <div className="absolute -top-1.5 -right-1.5 bg-gradient-to-r from-purple-500 to-pink-500 p-1 rounded-lg shadow-lg"><Sparkles className="w-3 h-3 text-white" /></div>
        </div>
        <h3 className="font-bold text-gray-800 text-lg sm:text-xl">Nenhum link ainda</h3>
        <p className="text-sm text-gray-500 mt-1.5 max-w-xs mx-auto">Adicione seu primeiro link e comece a compartilhar!</p>
        <div className="flex items-center justify-center gap-2 mt-4 text-purple-600"><MousePointerClick className="w-4 h-4 animate-bounce" /><span className="text-xs font-medium">Toque no botão abaixo</span></div>
      </div>
    </div>
  );
}

// 🔥 RECEBENDO O effectiveUserId AQUI 🔥
export default function ManageLinks({ effectiveUserId }: { effectiveUserId?: string }) {
  const { userId: clerkUserId } = useAuth();

  // O targetUserId será a sub-conta (se existir) ou a conta principal
  const targetUserId = effectiveUserId || clerkUserId;

  const links = useQuery(api.lib.links.getLinksByUserId, targetUserId ? { userId: targetUserId } : "skip");
  ...
  return (
    <div className="flex flex-col pb-4">
      ...
      <div className={clsx(
        ...
        >
          {/* 🔥 SE FOR SUB-CONTA, ELE PASSA A URL COM PARAMETRO PARA A PÁGINA NEW-LINK NÃO SE PERDER! 🔥 */}
          <Link
            href={`/dashboard/new-link${effectiveUserId ? `?subUserId=${effectiveUserId}` : ''}`}
