...
export default function ImageGeneratorPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950/20 to-slate-950">
      ...
      <motion.div initial={{ y: -100 }} animate={{ y: 0 }} className="relative border-b border-white/10 bg-black/20 backdrop-blur-xl sticky top-0 z-40">
                  ...
                  <form onSubmit={handleGenerate} className="space-y-4 sm:space-y-6">
                    <div>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-xs text-gray-500">{prompt.length}/500 caracteres</span>
                        {prompt && (
                          <Button type="button" variant="ghost" size="sm" onClick={() => setPrompt("")} className="text-xs h-6 px-2">
                    ...
                    {usageStats && (
                      <div className="p-3 sm:p-4 bg-gradient-to-r from-green-500/10 to-emerald-500/10 rounded-xl border border-green-500/20">
                        <div className="flex items-center justify-between text-xs sm:text-sm">
                          <span className="text-green-300">✨ {usageStats.remainingToday} gerações grátis hoje</span>
                          <span className="text-xs text-gray-400 hidden sm:inline">{usageStats.costPerImage}</span>
                        </div>
                      </div>
                    )}
                  </form>
                ...
                <Card className="p-4 sm:p-6 bg-white/5 backdrop-blur-xl border-white/10 shadow-2xl lg:sticky lg:top-20">
                  <div className="flex items-center justify-between mb-3 sm:mb-4">
                    ...
                    {latestImage && (
                      <div className="flex gap-1">
                        <Button size="icon" variant="ghost" onClick={() => handleDownload(latestImage, `ai-image-${Date.now()}.png`)} className="h-7 w-7 sm:h-8 sm:w-8">
                          <Download className="w-3 h-3 sm:w-4 sm:h-4" />
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => setSelectedImage(latestImage)} className="h-7 w-7 sm:h-8 sm:w-8">
                  ...
                  <div className="aspect-square bg-gradient-to-br from-purple-500/20 to-pink-500/20 rounded-2xl overflow-hidden relative">
                    {isGenerating ? (
                      ...
                      <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative w-full h-full">
                        <Image src={latestImage} alt="Generated" fill className="object-cover" />
                      </motion.div>
                    ) : (
          ...
          <TabsContent value="gallery">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 sm:space-y-6">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-white mb-1 sm:mb-2">Sua Galeria de Arte</h2>
                  <p className="text-sm sm:text-base text-gray-400">
                    {imageHistory.length} {imageHistory.length === 1 ? 'imagem' : 'imagens'} criadas
                    {totalPages > 1 && <span className="ml-2 text-purple-400">• Página {currentPage} de {totalPages}</span>}
                  </p>
                </div>
                {likedImages.size > 0 && (
              ...
              {imageHistory.length === 0 ? (
                <Card className="p-10 sm:p-20 bg-white/5 backdrop-blur-xl border-white/10 border-2 border-dashed text-center">
                  ...
                  <h3 className="text-lg sm:text-xl font-semibold text-white mb-2">Galeria vazia</h3>
                  <p className="text-sm sm:text-base text-gray-400 mb-4 sm:mb-6">Comece criando sua primeira obra-prima!</p>
                  <Button onClick={() => setActiveTab("create")} className="bg-gradient-to-r from-purple-600 to-pink-600 text-sm sm:text-base">
                ...
                <>
                        ...
                        >
                          <Card className="overflow-hidden bg-white/5 backdrop-blur-xl border-white/10 hover:border-purple-500/50 transition-all shadow-lg hover:shadow-2xl hover:shadow-purple-500/20">
                            <div className="relative aspect-square bg-gradient-to-br from-purple-500/20 to-pink-500/20">
                              <Image src={image.imageUrl} alt={image.prompt} fill className="object-cover" sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" />

                              <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/60 to-transparent opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-2 sm:p-4">
                                <p className="text-white text-xs mb-2 sm:mb-3 line-clamp-2 font-medium leading-tight">{image.prompt}</p>
                                <div className="flex items-center justify-between gap-1">
                                  <div className="flex gap-0.5 sm:gap-1">
                                    <Button size="icon" variant="ghost" onClick={() => toggleLike(image._id)} className="h-8 w-8 sm:h-9 sm:w-9 bg-white/10 hover:bg-white/20 backdrop-blur-sm">
                                      <Heart className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${likedImages.has(image._id) ? 'fill-red-500 text-red-500' : 'text-white'}`} />
                                    </Button>
                                    <Button size="icon" variant="ghost" onClick={() => handleDownload(image.imageUrl, `ai-${image._id}.png`)} className="h-8 w-8 sm:h-9 sm:w-9 bg-white/10 hover:bg-white/20 backdrop-blur-sm">
                                      <Download className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" />
                                    </Button>
                                    <Button size="icon" variant="ghost" onClick={() => handleShare(image.imageUrl, image.prompt)} className="h-8 w-8 sm:h-9 sm:w-9 bg-white/10 hover:bg-white/20 backdrop-blur-sm hidden sm:flex">
                                      <Share2 className="w-4 h-4 text-white" />
                                    </Button>
                                    <Button size="icon" variant="ghost" onClick={() => handleDelete(image._id, image.storageId)} className="h-8 w-8 sm:h-9 sm:w-9 bg-red-500/20 hover:bg-red-500/40 backdrop-blur-sm">
                                      <Trash2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" />
                                    </Button>
                                  </div>
                                  <Button size="icon" variant="ghost" onClick={() => setSelectedImage(image.imageUrl)} className="h-8 w-8 sm:h-9 sm:w-9 bg-white/10 hover:bg-white/20 backdrop-blur-sm">
                              ...
                              {likedImages.has(image._id) && (
                                <div className="absolute top-2 right-2 z-10">
                                  <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-7 h-7 sm:w-8 sm:h-8 bg-red-500 rounded-full flex items-center justify-center shadow-lg">
                  ...
                  {totalPages > 1 && (
                    <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="flex justify-center items-center gap-2 mt-8">
                      <Button onClick={() => goToPage(1)} disabled={currentPage === 1} variant="outline" size="icon" className="h-9 w-9 sm:h-10 sm:w-10 bg-white/5 border-white/10 hover:bg-white/10 disabled:opacity-30">
                        <ChevronsLeft className="w-4 h-4" />
                      </Button>
                      <Button onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1} variant="outline" size="icon" className="h-9 w-9 sm:h-10 sm:w-10 bg-white/5 border-white/10 hover:bg-white/10 disabled:opacity-30">
                          ...
                          .map((page, index, array) => {
                            ...
                            return (
                              <div key={page} className="flex items-center gap-1 sm:gap-2">
                                {showEllipsis && <span className="text-gray-500 px-1 text-sm sm:text-base">...</span>}
                                <Button
                      ...
                      <Button onClick={() => goToPage(currentPage + 1)} disabled={currentPage === totalPages} variant="outline" size="icon" className="h-9 w-9 sm:h-10 sm:w-10 bg-white/5 border-white/10 hover:bg-white/10 disabled:opacity-30">
                        <ChevronRight className="w-4 h-4" />
                      </Button>
                      <Button onClick={() => goToPage(totalPages)} disabled={currentPage === totalPages} variant="outline" size="icon" className="h-9 w-9 sm:h-10 sm:w-10 bg-white/5 border-white/10 hover:bg-white/10 disabled:opacity-30">
              ...
              {imageHistory.length > 0 && (
                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4 mt-6 sm:mt-8">
                  {[
                    ...
                    { label: 'Esta semana', value: imageHistory.filter((img: ImageDocument) => img.createdAt && Date.now() - img.createdAt < 7 * 24 * 60 * 60 * 1000).length, icon: TrendingUp, color: 'blue' },
                    { label: 'Hoje', value: imageHistory.filter((img: ImageDocument) => img.createdAt && new Date(img.createdAt).toDateString() === new Date().toDateString()).length, icon: Zap, color: 'green' },
                  ].map((stat, i) => (
        ...
        {selectedImage && (
          ...
          >
            <motion.button
              ...
              exit={{ scale: 0 }}
              onClick={() => setSelectedImage(null)}
              className="fixed top-3 right-3 sm:top-4 sm:right-4 z-[110] min-w-[48px] min-h-[48px] w-12 h-12 sm:w-14 sm:h-14 bg-red-500/90 hover:bg-red-600 active:bg-red-700 backdrop-blur-xl rounded-full flex items-center justify-center shadow-2xl shadow-red-500/50 border-2 border-white/20 transition-all duration-200 touch-manipulation"
            ...
            <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }} className="relative max-w-6xl w-full max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
              <div className="relative rounded-xl sm:rounded-2xl overflow-hidden shadow-2xl">
                <Image src={selectedImage} alt="Full view" width={1920} height={1080} className="w-full h-auto max-h-[85vh] object-contain" priority />
              </div>

              <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-[105] flex gap-2 sm:gap-3 bg-black/80 backdrop-blur-xl rounded-full p-2 sm:p-3 border border-white/20 shadow-2xl">
                <Button size="icon" variant="ghost" onClick={() => handleDownload(selectedImage, `image-${Date.now()}.png`)} className="h-11 w-11 sm:h-12 sm:w-12 text-white hover:bg-white/20 rounded-full" title="Download">
                  <Download className="w-5 h-5 sm:w-6 sm:h-6" />
                </Button>
                <Button size="icon" variant="ghost" onClick={() => handleShare(selectedImage, "Imagem criada com IA")} className="h-11 w-11 sm:h-12 sm:w-12 text-white hover:bg-white/20 rounded-full" title="Compartilhar">
