...
type ClickData = { id: number; timestamp: number; country: string | null; };
...
export default function ShortLinkDetailsPage() {
    ...
    const { user } = useUser();
    const [data, setData] = useState<PageData | undefined | null>(undefined); // undefined: loading, null: error/not found
    ...
    if (data === undefined) {
        return <div>Carregando detalhes do link...</div>; // Skeleton
    }

    if (data === null) {
        return <div>Link não encontrado ou acesso negado.</div>; // Página de erro
    }
